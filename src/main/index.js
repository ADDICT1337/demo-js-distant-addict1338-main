...
async function getFamilyMembers() {
  try {
    ...
    console.log("Ошибка при получении данных о членах семьи:", e);
    return [];
  }
}
...
async function updateFamilyMember(_, member) {
  try {
    // Обновляем основные данные члена семьи
    ...
    // Проверяем, есть ли уже запись в family_members_job
    ...
    if (member.job_title || member.employer || member.salary > 0) {
      ...
      // Если работа не указана, удаляем запись из family_members_job
